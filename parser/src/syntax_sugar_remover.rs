...
use program_structure::statement_builders::{
    build_declaration, build_log_call, build_initialization_block,
};
    ...
    match stm {
        Statement::MultSubstitution { meta, lhe, rhe, op, .. } => {
            if lhe.contains_anonymous_comp() {
                ...
                let error =
                    "Anonymous components only admit the use of the operator <==".to_string();
    ...
    match stm {
        Statement::MultSubstitution { meta, lhe, op, rhe } => {
            ...
            if substitutions.is_empty() {
                ...
                Result::Ok((
                    Statement::Block { meta, stmts: substitutions },
                    comp_declarations,
                    Vec::new(),
                    Vec::new(),
                ))
        ...
        Statement::IfThenElse { meta, cond, if_case, else_case } => {
            ...
            if else_case.is_none() {
                Result::Ok((
                    Statement::IfThenElse { meta, cond, if_case: b_if, else_case: Option::None },
                    if_comp_dec,
                    if_var_dec,
                    if_subs,
                ))
        ...
        Statement::While { meta, cond, stmt } => {
            let id_var_while = "anon_var_".to_string()
                + &file_lib.get_line(meta.start, meta.get_file_id()).unwrap().to_string()
                + "_"
            ...
            let b_while = if !comp_dec.is_empty() {
                ...
                let new_block =
                    Statement::Block { meta: meta.clone(), stmts: vec![body, subs_access] };
                Box::new(new_block)
            } else {
                Box::new(body)
            };
            Result::Ok((
                Statement::While { meta, cond, stmt: b_while },
                comp_dec,
                var_declarations,
                subs_out,
            ))
    ...
    match exp {
        AnonymousComp { meta, id, params, signals, names, is_parallel } => {
            ...
            if let Some(m) = names {
                ...
                for (operator, name) in m {
                    if operator != AssignOp::AssignConstraintSignal {
                        let error = "Anonymous components only admit the use of the operator <=="
                            .to_string();
            ...
            if outputs.len() == 1 {
                ...
                Result::Ok((
                    declarations,
                    vec![Statement::Block { meta: meta.clone(), stmts: seq_substs }],
                    out_exp,
                ))
            } else {
