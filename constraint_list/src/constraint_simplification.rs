...
fn eq_simplification(
    ...
    let constraints = Arc::new(Mutex::new(LinkedList::new()));
    let substitutions = Arc::new(Mutex::new(LinkedList::new()));
    let clusters = build_clusters(equalities, no_vars);
    let no_clusters = clusters.len();
    let single_clusters = Arc::new(Mutex::new(0));
    let aux_constraints = Arc::new(Mutex::new(vec![LinkedList::new(); no_clusters]));

    clusters.into_par_iter().enumerate().for_each(|(id, cluster)| {
        if Cluster::size(&cluster) == 1 {
            let (mut subs, cons) = eq_cluster_simplification(cluster, &forbidden, &field);
            aux_constraints.lock().unwrap()[id] = cons;
            substitutions.lock().unwrap().append(&mut subs);
            *single_clusters.lock().unwrap() += 1;
        } else {
    ...
    for id in 0..no_clusters {
        constraints.lock().unwrap().append(&mut aux_constraints.lock().unwrap()[id]);
    }
    log_substitutions(&substitutions.lock().unwrap(), substitution_log);
    (
        Arc::try_unwrap(substitutions).unwrap().into_inner().unwrap(),
        Arc::try_unwrap(constraints).unwrap().into_inner().unwrap(),
    )
}
